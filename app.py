...
# @st.cache_resource
# def load_model():
#     """Load model from uploaded file or download default"""
#     try:
#         # Create a container for the model upload section
#         with st.expander("🤖 Model Configuration", expanded=True):
#             st.markdown("""
#             ### Choose Your Model
#             You can either:
#             1. Upload your custom YOLOv8 model (.pt file)
#             2. Use the default YOLOv8n model (will be downloaded automatically)
#             """)
            
#             # Add file uploader for custom model
#             uploaded_model = st.file_uploader("Upload Custom YOLOv8 Model", type=["pt"])
...
#                 st.success(f"✅ Custom model uploaded successfully!")
#             else:
#                 # Use default model
#                 model_path = os.path.join(TEMP_DIR, 'yolov8n.pt')
#                 if not os.path.exists(model_path):
#                     if not download_default_model(model_path):
#                         raise Exception("Failed to download default model")
...
# # Add requests to requirements if not already added
...
#     model = load_model()
# except Exception as e:
#     st.error("❌ Could not load the model. Please check your internet connection or try uploading a custom model.")
...
class ModelWrapper:
    ...
    def predict(self, frame):
        ...
        try:
            ...
            return pred_df, rendered_img
            
        except Exception as e:
            st.error(f"Error in model prediction: {str(e)}")
            # Return empty DataFrame and original frame if prediction fails
...
# HELMET CLASS DETECTION
# ---------------------------
def find_helmet_classes(model):
    """Find helmet-related classes in the model"""
    try:
        # Get class names directly from the model
        class_names = list(model.get_names().values())
        
        # Look for helmet-related classes
        helmet_classes = [cls for cls in class_names if any(k in cls.lower() for k in HELMET_KEYWORDS)]
        
        # If no helmet classes found, use common person detection classes
        if not helmet_classes:
            helmet_classes = ['person']  # Default to person detection
            
        return helmet_classes
    except Exception as e:
        st.error(f"Error finding helmet classes: {str(e)}")
        return ['person']  # Fallback to person detection
...
def load_model():
    try:
        ...
        # Fallback to default YOLOv8n model
            ...
            return ModelWrapper('yolov8n.pt')
        else:
            raise FileNotFoundError("No YOLOv8 model file found")
    except Exception as e:
...
def detect_and_alert(frame, confidence_thresh):
    """Detect objects and check for violations"""
    try:
        # Convert frame to BGR if it's RGB (OpenCV expects BGR)
        ...
        detections, rendered_frame = model.predict(frame_bgr)
        
        violation_detected = False
        violation_reason = ""
        
        # Check for any detections above threshold
        for _, row in detections.iterrows():
            if row['confidence'] > confidence_thresh:
                class_name = row['name']
                
                # Check for safety gear violations (custom classes)
                if class_name in ["NO-Hardhat", "no-helmet", "no-hardhat"]:
                    violation_reason = "No Hardhat Detected"
                    violation_detected = True
                    break
                elif class_name in ["NO-Mask", "no-mask"]:
                    violation_reason = "No Mask Detected"
                    violation_detected = True
                    break
                    ...
                    # Define restricted zone (adjust coordinates as needed)
                    restricted_zone = (100, 100, 500, 400)
                    if (restricted_zone[0] < x_center < restricted_zone[2] and 
                        restricted_zone[1] < y_center < restricted_zone[3]):
                        violation_reason = "Person entered Restricted Zone"
                        violation_detected = True
                        break
        ...
        if violation_detected:
            log_violation(violation_reason, rendered_frame)
            return rendered_frame, True
        ...
        if detect_mode == "Webcam":
            ...
            def video_frame_callback(frame):
                try:
                    ...
                    st.error(f"Webcam processing error: {str(e)}")
            ...
            # Show available model classes for debugging
            with st.expander("🔍 Model Information", expanded=False):
                st.markdown("**Available Detection Classes:**")
                class_names = list(model.get_names().values())
                st.write(class_names)
                st.markdown("**Helmet-related Classes:**")
                st.write(HELMET_CLASSES)
                
                # Add test detection button
                if st.button("🧪 Test Model with Sample Image", use_container_width=True):
                    st.info("Testing model with a sample image...")
                    
                    try:
                        # Method 1: Test with blank image
                        test_img = np.ones((640, 640, 3), dtype=np.uint8) * 128  # Gray image
                        test_img_bgr = cv2.cvtColor(test_img, cv2.COLOR_RGB2BGR)
                        
                        # Test direct YOLO prediction
                        results = model.model(test_img_bgr)
                        ...
                        if results and len(results) > 0:
                            ...
                            st.write(list(model.model.names.values()))
                            
                            # Show detection results
                            if hasattr(result, 'boxes') and result.boxes is not None:
                                st.markdown("**Test Detections:**")
                                detection_data = []
                                ...
                                if detection_data:
                                    ...
                                    st.info("No objects detected in test image (expected for blank image)")
                            else:
                                st.info("No objects detected in test image (expected for blank image)")
                        else:
                            st.error("❌ No results returned from model test")
                            
                    except Exception as e:
                        st.error(f"❌ Model test failed: {str(e)}")
                        st.info("This might indicate a model loading issue.")
            
            img_file = st.file_uploader("Choose Image", type=['jpg', 'png', 'jpeg'], help="Upload an image to analyze for helmet violations")
            
            if img_file is not None:
                try:
                    # Convert uploaded file to PIL Image and ensure RGB format
                    img = Image.open(img_file).convert("RGB")
                    img_array = np.array(img)
                    
                    # Show image info for debugging
                    st.info(f"📊 Image Info: {img.size[0]}x{img.size[1]} pixels, Mode: {img.mode}")
                    ...
                    with col2:
                        ...
                        if st.button("Analyze Image", use_container_width=True):
                            with st.spinner("Processing image..."):
                                # Method 1: Direct YOLO prediction on numpy array
                                try:
                                    # Convert RGB to BGR for OpenCV compatibility
                                    img_bgr = cv2.cvtColor(img_array, cv2.COLOR_RGB2BGR)
                                    
                                    # Run YOLO prediction directly
                                    results = model.model(img_bgr)
                                    ...
                                    if results and len(results) > 0:
                                        ...
                                        if hasattr(result, 'boxes') and result.boxes is not None:
                                            for box in result.boxes:
                                                if float(box.conf) > confidence_thresh:
                                                    ...
                                                    if class_name in ["NO-Hardhat", "no-helmet", "no-hardhat"]:
                                                        st.warning("🚨 Violation Detected: No Hardhat!")
                                                        violation_detected = True
                                                        break
                                                    elif class_name in ["NO-Mask", "no-mask"]:
                                                        st.warning("🚨 Violation Detected: No Mask!")
                                                        violation_detected = True
                                                        break
                                                    elif class_name in ["person", "Person"]:
                                                        ...
                                                        if (100 < x_center < 500 and 100 < y_center < 400):
                                                            st.warning("🚨 Violation Detected: Person in Restricted Zone!")
                                                            violation_detected = True
                                                            break
                                        
                                        if not violation_detected:
                                            st.success("✅ No violations detected!")
                                            st.info("Image processed successfully. All safety protocols appear to be followed.")
                                            ...
                                            if hasattr(result, 'boxes') and result.boxes is not None:
                                                ...
                                                for box in result.boxes:
                                                    detection_data.append({
                                                        'Class': model.model.names[int(box.cls)],
                                                        'Confidence': f"{float(box.conf):.3f}",
                                                        'X_min': f"{float(box.xyxy[0][0]):.1f}",
                                                        'Y_min': f"{float(box.xyxy[0][1]):.1f}",
                                                        'X_max': f"{float(box.xyxy[0][2]):.1f}",
                                                        'Y_max': f"{float(box.xyxy[0][3]):.1f}"
                                                    })
                                        ...
                                        st.error("❌ No detection results returned from model")
                                        
                                except Exception as e:
                                    st.error(f"❌ Error in YOLO prediction: {str(e)}")
                                    st.info("Trying alternative method...")
                                    
                                    # Fallback: Use the original detect_and_alert function
                                    try:
                                        processed_img, alert = detect_and_alert(img_array, confidence_thresh)
                                        st.image(processed_img, caption="Detection Result (Fallback)", use_container_width=True)
                                        
                                        if alert:
                                            st.warning("🚨 Violation Detected!")
                                            st.error("Safety violation has been logged to the database.")
                                        else:
                                            st.success("✅ No violations detected!")
                                            st.info("Image processed successfully. All safety protocols appear to be followed.")
                                    except Exception as fallback_error:
                                        st.error(f"❌ Both methods failed: {str(fallback_error)}")
